...
This is an interactive graphing calculator written in C. It parses raw user input with a behind-the-scenes parsing tree. The calculator can handle common math operations such as sin, log, etc. The graph can be moved and re-scaled during the interactive command-line session.
